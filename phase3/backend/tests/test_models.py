...
def test_conversation_creation():
    convo = Conversation(user_id="user_2")
    assert convo.user_id == "user_2"
    assert convo.id is None
    assert convo.created_at is not None

def test_message_creation():
    msg = Message(conversation_id=1, user_id="user_2", role="user", content="hello")
